...
export const KEY_PREFIX = 'expo-typescript'
...
export async function login (options?: {connection: string}) {
  ...
  if (result.type === 'success') {
    ...
    if (params.error) {
      Alert.alert('Error', params.error_description || 'something went wrong while logging in')
    ...
    await AsyncStorage.setItem(
      EXPIRES_AT_KEY, (now() + (Number(expires_in) * 1000)).toString()
    )
  }
...
export async function facebookLogin () {
  return login({connection: 'facebook'})
}
...
export async function getAccessToken () {
  ...
  if (Number(expiresAt) > new Date().getTime()) {
    const accessToken = await AsyncStorage.getItem(ACCESS_TOKEN_KEY) || undefined
